...
import { Sparkles, Loader2, FileSearch, Plus } from "lucide-react";
...
interface AIArchitectDialogProps {
  ...
  onCreateCanvas?: (name: string, description?: string, tags?: string[]) => Promise<void>;
...
export function AIArchitectDialog({ 
  ...
  const [targetCanvasId, setTargetCanvasId] = useState<string | null>(canvasId || null);
  
  const [projectData, setProjectData] = useState<any>(null);

  // Sync target canvas with active canvas when dialog opens
  useEffect(() => {
    if (open && canvasId) {
  ...
  const handleGenerate = async (useCriticFeedback = false) => {
    ...
    try {
      ...
      // Handle creating new canvas if selected
      const isNewCanvas = targetCanvasId === '__new__';
      let actualCanvasId = targetCanvasId;
      
      if (isNewCanvas && onCreateCanvas) {
        const newCanvasName = `AI Architect - ${new Date().toLocaleDateString()}`;
        await onCreateCanvas(newCanvasName, "Generated by AI Architect");
        // After creation, the newest canvas should be auto-selected by the parent
        // We'll use null here and let the parent handle the switch
        actualCanvasId = null;
      } else if (targetCanvasId && targetCanvasId !== canvasId && onSelectCanvas) {
        // Switch to target canvas before generating
        onSelectCanvas(targetCanvasId);
      }
      
      const context: any = {
        description: finalDescription,
        drawEdges,
        existingNodes: isNewCanvas ? [] : existingNodes.map(n => ({ data: n.data, position: n.position })),
        existingEdges: isNewCanvas ? [] : existingEdges.map(e => ({ 
          source: e.source, 
          target: e.target, 
          data: e.data 
        })),
        ...
        projectId,
        shareToken,
        canvasId: isNewCanvas ? null : (actualCanvasId || canvasId || null),
      };
                ...
                <div className="space-y-1 md:space-y-2 pt-2 md:pt-4 border-t">
                  ...
                  {canvases.length > 0 && (
                    <div className="space-y-1 pt-2">
                      <Label className="text-xs">Target Canvas</Label>
                      <Select 
                        value={targetCanvasId || canvasId || ''} 
                        onValueChange={setTargetCanvasId}
                      >
                        <SelectTrigger className="h-8 text-xs">
                          <SelectValue placeholder="Current canvas" />
                        </SelectTrigger>
                        <SelectContent>
                          {canvases.map((c) => (
                            <SelectItem key={c.id} value={c.id} className="text-xs">
                              {c.name}
                              {c.id === canvasId && " (current)"}
                            </SelectItem>
                          ))}
