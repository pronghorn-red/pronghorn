...
export function DatabaseExplorer({
    ...
    <div className="h-full flex flex-col bg-[#1e1e1e]">
      <div className="p-2 border-b border-[#3e3e42] bg-[#252526] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[#cccccc]">Schema</span>
        </div>
        <Button
          variant="ghost"
          size="icon"
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="h-full flex flex-col">
        <div className="border-b border-border px-2 flex items-center justify-between bg-background">
          <TabsList className="h-10">
            ...
            {selectedTable && (
              <TabsTrigger value="table" className="gap-2 pr-1 text-xs">
                <Table2 className="h-4 w-4" />
                <span className="hidden sm:inline">{`${selectedTable.schema}.${selectedTable.table}`}</span>
                <span className="sm:hidden">{selectedTable.table}</span>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleCloseTable();
  ...
  // Results Panel Content (for mobile)
  const ResultsPanel = () => (
    <div className="h-full bg-background">
  ...
  // Agent Panel Content (placeholder)
  const AgentPanel = () => (
    <div className="h-full flex flex-col bg-card">
      ...
      <div className="flex-1 flex items-center justify-center p-4 text-center">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Database Agent coming soon</p>
          <p className="text-xs text-muted-foreground/70">
            AI-powered SQL generation, schema analysis, and data exploration
          </p>
        </div>
      </div>
    </div>
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="p-2 border-b border-border bg-background flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <Database className="h-4 w-4 text-primary" />
            <span className="font-semibold text-sm truncate">{database.name}</span>
          </div>
        </div>

        {/* Tab Navigation */}
        <Tabs value={mobileActiveTab} onValueChange={setMobileActiveTab} className="flex-1 flex flex-col min-h-0">
          <TabsList className="w-full h-10 rounded-none border-b grid grid-cols-4">
          ...
          <TabsContent value="schema" className="flex-1 m-0 min-h-0" forceMount data-state={mobileActiveTab === "schema" ? "active" : "inactive"}>
            <div className={mobileActiveTab === "schema" ? "h-full" : "hidden"}>
              <SchemaTreePanel />
            </div>
          </TabsContent>

          <TabsContent value="query" className="flex-1 m-0 min-h-0" forceMount data-state={mobileActiveTab === "query" ? "active" : "inactive"}>
            <div className={mobileActiveTab === "query" ? "h-full" : "hidden"}>
              <SqlQueryEditor
                onExecute={handleExecuteQuery}
                isExecuting={isExecuting}
                initialQuery={currentQuery}
          ...
          <TabsContent value="results" className="flex-1 m-0 min-h-0" forceMount data-state={mobileActiveTab === "results" ? "active" : "inactive"}>
            <div className={mobileActiveTab === "results" ? "h-full" : "hidden"}>
              <ResultsPanel />
            </div>
          </TabsContent>

          <TabsContent value="agent" className="flex-1 m-0 min-h-0" forceMount data-state={mobileActiveTab === "agent" ? "active" : "inactive"}>
            <div className={mobileActiveTab === "agent" ? "h-full" : "hidden"}>
              <AgentPanel />
            </div>
          </TabsContent>
        </Tabs>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0">
        <ResizablePanelGroup direction="horizontal">
          ...
          <ResizablePanel defaultSize={isAgentPanelCollapsed ? 80 : 55} minSize={35}>
            <QueryEditorPanel />
          </ResizablePanel>

          {/* Right Panel - Agent (collapsible) */}
          {!isAgentPanelCollapsed && (
            <>
              <ResizableHandle withHandle />
