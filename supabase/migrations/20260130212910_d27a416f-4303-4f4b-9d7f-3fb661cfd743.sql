-- Fix update_deployment_with_token to handle empty string as "clear the value"
...
BEGIN
  ...
  UPDATE public.project_deployments SET
    ...
    -- Handle empty string as explicit clear, null means keep existing
    install_command = CASE 
      WHEN p_install_command = '' THEN NULL 
      ELSE COALESCE(p_install_command, install_command) 
    END,
    updated_at = now()
  WHERE id = p_deployment_id
